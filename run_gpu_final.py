...
def compute_metrics(y_true, y_pred, eval_mask):
    ...
    for thr in np.arange(0.02, 0.90, 0.02):
        ...
        f1 = 2 * p * r / (p + r + 1e-9)
        if f1 > best[0]: best = (f1.item(), thr, p.item(), r.item())
    try:
...
def load_data(name):
    ...
    eval_mask = np.array(mgr.to_eval, dtype=bool); terms = train.terms; adj = mgr.a
...
for ds_name in DATASETS:
    ...
    all_results.append({"method": "global", "dataset": ds_name,
                        "micro_f1": f1_g, "auprc": au_g, "precision": p_g,
                        "recall": r_g, "threshold": th_g,
                        "duration_s": dur_g, "epochs": epochs_g})
    ...
    mlp = TabularMLPModel(
        ...
        n_blocks=3, head_layers=2, dropout=0.3,
    ).to(DEVICE)
    ...
    all_results.append({"method": "tabular_mlp", "dataset": ds_name,
                        "micro_f1": f1_m, "auprc": au_m, "precision": p_m,
                        "recall": r_m, "threshold": th_m,
                        "duration_s": dur_m, "epochs": epochs_m})
    print(f"  => tabular_mlp   F1={f1_m:.4f}  AUPRC={au_m:.4f}  "
    ...
    all_results.append({"method": "gbdt_platt_r", "dataset": ds_name,
                        "micro_f1": f1_b, "auprc": au_b, "precision": p_b,
                        "recall": r_b, "threshold": th_b,
                        "duration_s": dur_b, "nodes": int(trainable.sum())})
    print(f"  => gbdt+platt+r  F1={f1_b:.4f}  AUPRC={au_b:.4f}  "
...
for ds_name in DATASETS:
    ...
    for r in sorted(ds_res, key=lambda x: -x["micro_f1"]):
        ...
        extra = f" ep={r.get('epochs','?')}" if 'epochs' in r else f" n={r.get('nodes','?')}"
        print(f"  {r['method']:<16} F1={r['micro_f1']:.4f}  "
...
for method in ["global", "gbdt_platt_r", "tabular_mlp"]:
    ...
    wins = sum(1 for ds in DATASETS
               ...
               and abs(r["micro_f1"] - max(rr["micro_f1"] for rr in all_results if rr["dataset"] == ds)) < 0.001)
    print(f"  {method:<16} F1={avg_f1:.4f}  AUPRC={avg_au:.4f}  "
