...
SEED = 42; DEVICE = torch.device("cuda")
...
mgr = initialize_dataset_experiments("wos", device="cpu", dataset_path="./data",
...
# Build R-matrix
r_np = np.zeros(a.shape); np.fill_diagonal(r_np, 1)
g_nx = nx.DiGraph(a)
for i in range(len(a)):
    ancestors = list(nx.descendants(g_nx, i))
    if ancestors: r_np[i, ancestors] = 1
...
for fname, tlist in [("WebOfScience_train.json", train_texts),
...
print(f"  Train texts: {len(train_texts)}  Test texts: {len(test_texts)}  Nodes: {n_nodes}")

# ===== 3. Model =====
from hmc.models.global_classifier.e2e.model import E2EConstrainedModel
model = E2EConstrainedModel(
    model_name="allenai/specter2_base", output_dim=n_nodes,
    r_matrix=r_matrix, hidden_dim=512, num_layers=3, dropout=0.3,
...
for ep in range(epochs):
    ...
    for i in range(0, n_train, batch_size):
        ...
        if n_batches % 200 == 0:
            print(f"  ep {ep+1}/{epochs} batch {n_batches}/{n_train//batch_size} loss={total_loss/n_batches:.4f}")
