...
def run_gbdt_platt(name, X_tr, y_tr, X_va, y_va, X_te, y_te,
                    eval_mask, hier):
...
for ds_name in DATASETS:
    ...
    f1_fin, au_fin, p_fin, r_fin, th_fin = compute_metrics(y_te, scores_final, eval_mask)

    print(f"  GBDT raw:       F1={f1_raw:.4f}  AUPRC={au_raw:.4f}  "
          f"P={p_raw:.4f}  R={r_raw:.4f}  thr={th_raw:.2f}")
    print(f"  GBDT + Platt:   F1={f1_cal:.4f}  AUPRC={au_cal:.4f}  "
          f"P={p_cal:.4f}  R={r_cal:.4f}  thr={th_cal:.2f}")
    print(f"  GBDT + Platt+R: F1={f1_fin:.4f}  AUPRC={au_fin:.4f}  "
          f"P={p_fin:.4f}  R={r_fin:.4f}  thr={th_fin:.2f}")
    print(f"  Time: {dur:.1f}s  Nodes trained: {n_trained}/{n_nodes}")

    all_results.append({
        "method": "gbdt_raw", "dataset": ds_name,
        "micro_f1": float(f1_raw), "auprc": float(au_raw),
        "precision": float(p_raw), "recall": float(r_raw),
        "threshold": float(th_raw), "duration_s": dur,
    })
    all_results.append({
        "method": "gbdt_platt", "dataset": ds_name,
        "micro_f1": float(f1_cal), "auprc": float(au_cal),
        "precision": float(p_cal), "recall": float(r_cal),
        "threshold": float(th_cal), "duration_s": dur,
    })
    all_results.append({
        "method": "gbdt_platt_r", "dataset": ds_name,
        "micro_f1": float(f1_fin), "auprc": float(au_fin),
        "precision": float(p_fin), "recall": float(r_fin),
        "threshold": float(th_fin), "duration_s": dur,
    })
...
try:
    ...
    all_results.extend([r for r in prev if r["method"] in ("global", "tabular_gbdt", "tabular_mlp")])
except Exception:
    pass
...
for method in ["global", "gbdt_platt_r", "gbdt_platt", "tabular_gbdt", "gbdt_raw", "tabular_mlp"]:
