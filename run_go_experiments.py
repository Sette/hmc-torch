...
def train_and_eval_go(X_tr, y_tr, X_te, y_te, eval_mask, graph,
                       epochs=20, seed=42):
    ...
    n_feat = X_tr.shape[1]; n_nodes = y_tr.shape[1]
    t0 = time.time()

    # Build sparse R for inference-time reconciliation
    sparse_r = BlockDiagonalR(graph)
    print(f"    SparseR: {sparse_r.n_nodes} nodes, {len(sparse_r.levels)} levels, "
    ...
    model = ConstrainedModel(
        input_dim=n_feat, hidden_dim=min(512, n_feat), output_dim=n_nodes,
        hyperparams={"batch_size": BATCH, "num_layers": 2, "dropout": 0.3,
                     "non_lin": "relu", "hidden_dim": min(512, n_feat),
    ...
    return (f1_raw, au_raw, p_raw, r_raw, th_raw,
            f1_rec, au_rec, p_rec, r_rec, th_rec, dur)
...
for ds_name in GO_DATASETS:
    ...
    try:
        ...
        print(f"  {X_tr.shape[0]} train | {X_te.shape[0]} test | "
              f"{X_tr.shape[1]} feat | {n_nodes} nodes | eval={eval_mask.sum()}")

        (f1_raw, au_raw, p_raw, r_raw, th_raw,
         f1_rec, au_rec, p_rec, r_rec, th_rec, dur) = train_and_eval_go(
            X_tr, y_tr, X_te, y_te, eval_mask, g, epochs=20, seed=42)

        delta_f1 = f1_rec - f1_raw
        delta_au = au_rec - au_raw
        print(f"  Raw:      F1={f1_raw:.4f} AUPRC={au_raw:.4f} P={p_raw:.4f} R={r_raw:.4f}")
        print(f"  Sparse R: F1={f1_rec:.4f} AUPRC={au_rec:.4f} P={p_rec:.4f} R={r_rec:.4f}")
        print(f"  Δ: F1={delta_f1:+.4f} AUPRC={delta_au:+.4f} t={dur:.1f}s")

        all_results.append({
            "dataset": ds_name, "nodes": n_nodes,
            "raw_F1": f1_raw, "raw_AUPRC": au_raw,
            "raw_P": p_raw, "raw_R": r_raw,
            "sparse_F1": f1_rec, "sparse_AUPRC": au_rec,
            "sparse_P": p_rec, "sparse_R": r_rec,
            "delta_F1": delta_f1, "delta_AUPRC": delta_au,
            "duration_s": dur,
        })
...
print("=" * 70)
print(f"{'Dataset':<18} {'Nodes':>6} {'Raw F1':>10} {'Sparse F1':>10} {'ΔF1':>8} {'ΔAUPRC':>8}")
print("-" * 70)
for r in all_results:
    if "error" in r:
        print(f"{r['dataset']:<18} ERROR: {r['error'][:40]}")
    else:
        print(f"{r['dataset']:<18} {r['nodes']:>6} {r['raw_F1']:>10.4f} "
