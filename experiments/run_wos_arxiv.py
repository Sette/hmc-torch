...
def train_global(name, X_tr, y_tr, X_te, y_te, eval_mask, adj, n_nodes, epochs=30):
    ...
    preds = []
    te_ldr = DataLoader(
        TensorDataset(torch.tensor(X_te)), batch_size=128, shuffle=False
    )
    with torch.no_grad():
        for (bx,) in te_ldr:
...
try:
    ...
    print(
        f"  Train: {X_tr.shape}  Test: {X_te.shape}  Nodes: {n_nodes}  Eval: {eval_mask.sum()}"
    )
    f1, au, p, r, thr, dur, scores = train_global(
        "wos", X_tr, y_tr, X_te, y_te, eval_mask, adj, n_nodes, epochs=50
    )
    results.append(
        {
            "dataset": "WOS",
    ...
    print(
        f"  => F1={f1:.4f}  AUPRC={au:.4f}  P={p:.4f}  R={r:.4f}  thr={thr:.2f}  t={dur:.1f}s"
    )
...
try:
    ...
    f1, au, p, r, thr, dur, scores = train_global(
        "arxiv", X_tr, y_tr, X_te, y_te, eval_mask, adj, n_nodes, epochs=50
    )
    results.append(
        {
            "dataset": "ArXiv",
