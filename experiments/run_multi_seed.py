...
mgr_w = initialize_dataset_experiments(
    "wos",
    device="cpu",
    dataset_path="./data",
    is_global=True,
...
Xw_te = te_w.x.astype(np.float32)
yw_te = te_w.y.astype(np.float32)
...
rw = np.zeros(aw.shape)
np.fill_diagonal(rw, 1)
gnx = nx.DiGraph(aw)
...
for seed in [0, 42, 123]:
    ...
    tr_ldr = DataLoader(
        TensorDataset(torch.tensor(Xw), torch.tensor(yw)), batch_size=128, shuffle=True
    )
    model.train()
    for ep in range(50):
        for bx, by in tr_ldr:
    ...
    preds = []
    te_ldr = DataLoader(
        TensorDataset(torch.tensor(Xw_te)), batch_size=64, shuffle=False
    )
    with torch.no_grad():
        for (bx,) in te_ldr:
...
for ds_name in ["cellcycle_FUN", "seq_FUN"]:
    ...
    Xf_te = te.x.astype(np.float32)
    yf_te = te.y.astype(np.float32)
    ef = np.array(mgr.to_eval, dtype=bool)
    af = mgr.a
    nf = len(mgr.nodes_idx)
    ...
    rf = np.zeros(af.shape)
    np.fill_diagonal(rf, 1)
    gnx2 = nx.DiGraph(af)
    ...
    for seed in [0, 42, 123]:
        ...
        preds = []
        te_ldr = DataLoader(
            TensorDataset(torch.tensor(Xf_te)), batch_size=64, shuffle=False
        )
        with torch.no_grad():
            for (bx,) in te_ldr:
                preds.append(model(bx.to(DEVICE)).cpu().numpy())
...
for ds_name in ["cellcycle_GO", "expr_GO"]:
    ...
    Xg_te = te.x.astype(np.float32)
    yg_te = te.y.astype(np.float32)
    ...
    for seed in [0, 42, 123]:
        ...
        opt = torch.optim.AdamW(model.parameters(), lr=1e-4, weight_decay=1e-5)
        tr_ldr = DataLoader(
            TensorDataset(torch.tensor(Xg), torch.tensor(yg)),
            batch_size=64,
            shuffle=True,
        )
        model.train()
        ...
        preds = []
        te_ldr = DataLoader(
            TensorDataset(torch.tensor(Xg_te)), batch_size=32, shuffle=False
        )
        with torch.no_grad():
            for (bx,) in te_ldr:
                preds.append(model(bx.to(DEVICE)).cpu().numpy())
        ...
        scores_rec = (
            sparse_r.reconcile(torch.tensor(scores_raw).to(DEVICE)).cpu().numpy()
        )
...
print("=" * 70)
for name, data in [
    ("ArXiv", all_results["arxiv_3seeds"]),
