...
def compute_metrics(y_true, y_pred, eval_mask):
    ...
    for thr in np.arange(0.1, 0.90, 0.05):
        ...
        f1 = 2 * p * r / (p + r + 1e-9)
        if f1 > best_f1:
            best_f1, best_thr, best_p, best_r = f1.item(), thr, p.item(), r.item()
    try:
        auprc = float(
...
for ds_name in DATASETS:
    ...
    with torch.no_grad():
        te_ldr_m = DataLoader(
            TensorDataset(torch.tensor(X_te)), batch_size=BATCH, shuffle=False
        )
        preds_m = []
        for (bx,) in te_ldr_m:
    ...
    print(
        f"  => mlp        F1={f1_m:.4f}  AUPRC={au_m:.4f}  "
    ...
    from hmc.datasets.manager.dataset_manager import (
        initialize_dataset_experiments as init_ds,
    )
    ...
    X_tr2 = scaler2.fit_transform(
        imp2.fit_transform(np.concatenate([tr2.x, va2.x]).astype(np.float32))
    ).astype(np.float32)
    y_tr2 = np.concatenate([tr2.y, va2.y]).astype(np.float32)
    X_va2 = scaler2.transform(imp2.transform(va2.x.astype(np.float32))).astype(
        np.float32
    )
    y_va2 = va2.y.astype(np.float32)
    X_te2 = scaler2.transform(imp2.transform(te2_.x.astype(np.float32))).astype(
        np.float32
    )
    ...
    print(
        f"  => gbdt+platt F1={f1_b:.4f}  AUPRC={au_b:.4f}  "
...
for method in ["global", "gbdt_platt_r", "tabular_mlp"]:
    ...
    print(
        f"  {method:<18} avg F1={avg_f1:.4f}  avg AUPRC={avg_au:.4f}  "
