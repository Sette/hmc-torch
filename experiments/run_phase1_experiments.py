...
print(
    f"  Train: {Xa_tr.shape}  Test: {Xa_te.shape}  Nodes: {n_a}  Eval: {eval_a.sum()}"
)
...
print(
    f"  ArXiv: F1={np.mean(f1s):.4f}±{np.std(f1s):.4f}  AUPRC={np.mean(aus):.4f}±{np.std(aus):.4f}"
)
...
print(
    f"  WOS: F1={np.mean(f1s_w):.4f}±{np.std(f1s_w):.4f}  AUPRC={np.mean(aus_w):.4f}±{np.std(aus_w):.4f}"
)
...
for use_r in [True, False]:
    ...
    ablation_results[f"arxiv_{label}"] = {
        "f1": f1,
        "auprc": au,
        "p": p,
        "r": r,
...
for ds_name in ["cellcycle_FUN", "seq_FUN"]:
    ...
    imp_f = SimpleImputer(strategy="mean")
    ...
    for use_r in [True, False]:
        ...
        ablation_results[f"{ds_name}_{label}"] = {
            "f1": f1,
            "auprc": au,
            "p": p,
            "r": r,
...
for ds in ["arxiv", "wos", "cellcycle_FUN", "seq_FUN"]:
    ...
    print(
        f"  {ds:<20} R=on F1={on['f1']:.4f}  R=off F1={off['f1']:.4f}  "
