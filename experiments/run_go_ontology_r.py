...
class OntologyEnhancedModel(nn.Module):
    ...
    def _build_edge_index(self, graph):
        ...
        idx = list(edges)
        return torch.tensor(
            [[i for i, j in idx], [j for i, j in idx]], dtype=torch.long
        )
...
def run_experiment(
    ds_name, X_tr, y_tr, X_te, y_te, eval_mask, graph, n_nodes, epochs=30, alpha=0.3
):
...
for ds_name in ["cellcycle_GO", "eisen_GO"]:
    ...
    print(
        f"  {X_tr.shape[0]} train | {X_te.shape[0]} test | {X_tr.shape[1]} feat | {n_nodes} nodes"
    )
    ...
    for alpha in [0.1, 0.3, 0.5]:
        ...
        results[f"{ds_name}_α={alpha}"] = {
            "f1_raw": float(f1_raw),
            "auprc_raw": float(au_raw),
            "f1_rec": float(f1_rec),
            "auprc_rec": float(au_rec),
            "time": dur,
        }
        print(
            f"  α={alpha}: raw F1={f1_raw:.4f} AUPRC={au_raw:.4f} → "
...
for ds_name in ["cellcycle_GO", "eisen_GO"]:
    ...
    model = ConstrainedModel(
        ...
        hidden_dim=min(512, X_tr.shape[1]),
        output_dim=n_nodes,
        hyperparams={
            "batch_size": 64,
            "num_layers": 3,
    ...
    with torch.no_grad():
        for (bx,) in DataLoader(
            TensorDataset(torch.tensor(X_te)), batch_size=32, shuffle=False
        ):
            preds.append(model(bx.to(DEVICE)).cpu().numpy())
    scores_raw = np.concatenate(preds, axis=0)
    scores_rec = sparse_r.reconcile(torch.tensor(scores_raw).to(DEVICE)).cpu().numpy()
    ...
    print(
        f"  {ds_name} MLP: raw F1={f1_raw:.4f} AUPRC={au_raw:.4f} → "
        f"+R F1={f1_rec:.4f} AUPRC={au_rec:.4f} t={dur:.1f}s"
    )
...
sota = {
    "cellcycle_GO": {"HMCN-F": 0.400, "C-HMCNN": 0.413, "WWW'22": 0.460},
...
for ds_name in ["cellcycle_GO", "eisen_GO"]:
    ...
    best_ont = max(
        (k, v) for k, v in results.items() if ds_name in k and "baseline" not in k
    )
