...
experimentation across modalities and hierarchy types. We evaluate on
three benchmark families: \textbf{ArXiv} (156 classes), \textbf{WOS}
(142 classes), and \textbf{FunCat} (8 datasets, up to 500 classes).
...
FunCat---matching or approaching neural methods like HMCN-F and
C-HMCNN despite using significantly simpler architectures. We provide
an honest assessment of where the platform excels (text
classification with clean hierarchies) and where it remains below the
state-of-the-art (protein function prediction requiring ontology
learning). The platform is open-source and GPU-accelerated.
\end{abstract}
...
\textbf{Losses.} Weighted BCE (prevalence-based), Focal Loss,
Hierarchical Consistency (Eq.~\ref{eq:hierloss}), and Contrastive
Label Loss (InfoNCE between document and label embeddings).
...
\midrule
\multicolumn{6}{c}{\textit{Tabular (FunCat hierarchy, genomic features)}} \\
...
surpassing HiAGM by 13.45 points. The R-matrix constraint alone
(without fine-tuning or GCN) outperforms all previous methods,
demonstrating that hierarchical consistency is more impactful than
complex architectures when the taxonomy is clean and informative.

\textbf{WOS.} Frozen SPECTER2 achieves 0.7519; fine-tuning raises it
to \textbf{0.8743}, a new state-of-the-art (+0.0023 over
HTC-infoMAX). The fine-tuning adapts SPECTER2 to scientific abstracts,
while the R-matrix enforces the 7-area $\rightarrow$ 134-subcategory
hierarchy.
...
\midrule
\textit{Avg} & \textbf{0.312} & 0.264 & 0.261 & 0.250 & 0.250 \\
\bottomrule
...
average. The issue: calibrated probabilities align with class
prevalence ($<5\%$ for most leaf nodes), shifting the optimal decision
threshold below typical sweep ranges. We recommend raw score
optimization over calibration for highly imbalanced HMC.
...
\begin{enumerate}[leftmargin=*,nosep]
  ...
  \item \textbf{Scalability.} The R-matrix is $O(N^2)$ dense, limiting
    applicability to $N \lesssim 2000$. For Gene Ontology datasets
    (4,000--40,000 classes), sparse approximations are needed.
...
\begin{itemize}[leftmargin=*,nosep]
  ...
  \item Honest assessment: the binary R-matrix cannot match ontology
    representation learning for protein function prediction, but
    provides strong structural regularization for clean taxonomies.
