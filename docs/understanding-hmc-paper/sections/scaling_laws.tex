...
number of classes $C$ (log scale). \textbf{(b)} Micro-F1 vs.\ hierarchy
depth $D$. \textbf{(c)} Training time vs.\ problem size $C \times N$ (log-log)
with power-law fit $t \propto (C{\times}N)^{0.85}$ ($R^2{=}0.94$).}
\label{fig:scaling}
\end{figure}
