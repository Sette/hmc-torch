...
\midrule
\ours{C-HMCNN + SPECTER2} & SPECTER2 (768\,d, frozen)
    & \ours{--} & este trabalho \\
\midrule
...
\begin{table}[htbp]
\centering
\caption{Configurações do \texttt{hmc-torch} para estudo de ablação no ArXiv.
         Ganhos estimados com base na literatura de WOS.}
\label{tab:arxiv_ablation}
\begin{tabular}{llc}
\toprule
\textbf{Configuração} & \textbf{Método} & \textbf{Ganho est.\ (Micro-F1)} \\
\midrule
\ours{Baseline} & SPECTER2 frozen + MLP + R-matrix & referência \\
+ Fine-tuning  & \texttt{globalE2E}                 & $\approx$\,+5\,pp \\
+ Label GNN    & \texttt{globalSOTA}                 & $\approx$\,+4--6\,pp \\
+ Contrastivo  & \texttt{--use\_contrastive\_loss}   & $\approx$\,+2--4\,pp \\
\bottomrule
\end{tabular}
\end{table}
...
Com base na diferença entre nossa configuração \textit{baseline} e os resultados
publicados no WOS, identificamos três fatores principais:
...
\begin{enumerate}[leftmargin=*, itemsep=2pt]
  \item \textbf{WOS pendente.} Os resultados no WOS ainda precisam ser executados
        (indicados como ``--'' na Tabela~\ref{tab:wos_sota}). O pipeline está
...
\textbf{72,6\%} de Micro-F1 e \textbf{81,0\%} de Avg.\ Precision, com o
nível~0 (áreas) em 81,7\% e o nível~1 (subcategorias) em 57,4\% de F1.
No \textbf{WOS}, o mesmo pipeline está implementado e pronto para execução,
permitindo comparação direta com HiAGM (85,8\%), HGCLR (87,1\%) e HPT (87,2\%).
