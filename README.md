...
The ArXiv benchmark in this repository uses `arxiv-metadata-oai-snapshot.json`, title + abstract text, 50 K records, an 80/10/10 split with seed 42, and two active hierarchy levels after the root: 19 top-level areas and 137 subcategories. Because ArXiv HTC/HMC papers use different subsets, splits, label depths, and metrics, the external numbers below are approximate ranges for orientation rather than a strict leaderboard.

### Our runs

| Method | Features | Architecture | Micro-F1 | Avg. Precision |
|---|---|---|---:|---:|
| `global` | SPECTER2 frozen (768d) | MLP + R-matrix | **72.6%** | **81.0%** |

Per-level F1:

| Features | Level 0: areas | Level 1: subcategories |
|---|---:|---:|
| SPECTER2 frozen (768d) | **81.7%** | **57.4%** |

SPECTER2 is the default ArXiv representation. The model uses scientific transformer embeddings for title + abstract and CLS pooling for SPECTER-family encoders.

### Comparison with state of the art

| Method | Typical encoder / features | Architecture family | Micro-F1 range | Reference |
|---|---|---|---:|---|
| **hmc-torch `global`** | SPECTER2 frozen | MLP + R-matrix | **72.6%** | this repository |
| HiAGM | BERT | Text encoder + hierarchy-aware label graph | 82-87% | [Zhou et al., ACL 2020](https://aclanthology.org/2020.acl-main.104/) |
| HGCLR | BERT | Hierarchy-guided contrastive learning | 85-89% | [Wang et al., ACL 2022](https://arxiv.org/abs/2203.03825) |
| HPT | BERT / RoBERTa | Hierarchy-aware prompt tuning | 87-91% | [Wang et al., EMNLP 2022](https://arxiv.org/abs/2204.13413) |

The current best repository result remains roughly **15 pp below** the strongest transformer-based HTC methods. The most direct path to close the gap is to use the already exposed `globalE2E` / `globalSOTA` pipelines: fine-tune the transformer, add label-graph encoding, and keep hierarchical constraint enforcement at inference time. See [docs/results_report.tex](docs/results_report.tex) and [docs/arxiv-hmc-sota.md](docs/arxiv-hmc-sota.md) for the full analysis.
...
│   │   │   └── manager.py             # ArXivManager — transformer embedding features
│   │   └── gofun/
...
│   └── results_report.tex             # ArXiv results report and SOTA comparison
│
...
The ArXiv dataset uses a two-level category hierarchy (e.g. `cs` → `cs.AI`, `cs.LG`) and transformer embeddings via HuggingFace. The embedding model is controlled by `--arxiv_model_name`.

Place the dataset file at `data/arxiv/arxiv-metadata-oai-snapshot.json` (see [Datasets](#datasets)), then:

```bash
# Default: SPECTER2 embeddings, MLP + R-matrix
...
> **Feature cache:** ArXiv embeddings are cached to `data/arxiv/.feature_cache/<hash>.npy`. The hash encodes the JSONL path, file mtime/size, `model_name`, and number of loaded records — the cache is invalidated automatically if any of those change.
>
> **Note:** the first run downloads model weights from the HuggingFace Hub (~500 MB for SPECTER2). Encoding 50 K documents takes a few minutes on CPU; use `--device cuda` to speed it up.
...
| `--arxiv_model_name` | `allenai/specter2_base` | HuggingFace model for ArXiv embeddings / E2E modes |
