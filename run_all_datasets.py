...
def load_data(name, with_adj=True):
    ...
    mgr = initialize_dataset_experiments(name, device="cpu", dataset_path="./data",
                                          dataset_type="arff", is_global=True if with_adj else False)
...
all_results = []
ALL_DS = (
    ["cellcycle_FUN","derisi_FUN","eisen_FUN","expr_FUN",
     "gasch1_FUN","gasch2_FUN","seq_FUN","spo_FUN","church_FUN","pheno_FUN"] +
    ["cellcycle_GO","derisi_GO","eisen_GO","expr_GO",
     "gasch1_GO","gasch2_GO","seq_GO","spo_GO","pheno_GO"] +
...
for ds_name in ALL_DS:
    ...
    try:
        ...
        all_results.append({"method": "global", "dataset": ds_name,
                            "micro_f1": f1, "auprc": au, "precision": p,
                            "recall": r, "threshold": thr,
                            "duration_s": dur, "epochs": epochs,
                            "n_nodes": n_nodes, "n_feat": n_feat,
                            "use_r_matrix": use_r})
...
print("=" * 70)
for group, label in [("_FUN","FunCat"), ("_GO","Gene Ontology"), ("_others","Others")]:
    ...
    for r in sorted(ds_list, key=lambda x: -(x.get("micro_f1") or 0)):
        if "error" in r:
            ...
            print(f"  {r['dataset']:<22} F1={r['micro_f1']:.4f}  "
                  f"AUPRC={r['auprc']:.4f}  t={r['duration_s']:.1f}s  "
                  f"R={r.get('use_r_matrix','?')}")
    if f1s:
        print(f"  {'AVG':<22} F1={np.mean(f1s):.4f}  AUPRC={np.mean([r['auprc'] for r in ds_list if 'auprc' in r]):.4f}")
