...
mgr = initialize_dataset_experiments(
    "seq_FUN", device="cpu", dataset_path="./data",
...
dur = time.time() - t0
results.append({"method": "tabular_gbdt", "dataset": "seq_FUN",
...
print(f"  F1={f1:.4f}  AUPRC={au:.4f}  P={p:.4f}  R={r:.4f}  "
      f"thr={thr:.2f}  time={dur:.1f}s  nodes={gbdt_mask.sum()}")

save_artefacts(f"{base_dir}/tabular_gbdt", scores_gbdt_raw, scores_gbdt,
               results[-1], {"method": "tabular_gbdt", "dataset": "seq_FUN",
                             "backend": "histgb", "seed": 42,
                             "nodes_trained": int(gbdt_mask.sum()),
...
model_mlp = TabularMLPModel(
    input_dim=n_features, n_nodes=n_nodes,
    hidden_dim=512, n_blocks=3, head_layers=2, dropout=0.3,
).to(device)
...
dur = time.time() - t0
results.append({"method": "tabular_mlp", "dataset": "seq_FUN",
                "micro_f1": float(f1), "auprc": float(au),
                "precision": float(p), "recall": float(r),
                "threshold": float(thr), "duration_s": dur})
print(f"  F1={f1:.4f}  AUPRC={au:.4f}  P={p:.4f}  R={r:.4f}  "
...
save_artefacts(f"{base_dir}/tabular_mlp", scores_mlp_raw, scores_mlp,
               results[-1], {"method": "tabular_mlp", "dataset": "seq_FUN",
                             "epochs": epochs, "seed": 0,
                             "hidden_dim": 512, "n_blocks": 3, "dropout": 0.3})
...
dur = time.time() - t0
results.append({"method": "local", "dataset": "seq_FUN",
                "micro_f1": float(f1), "auprc": float(au),
                "precision": float(p), "recall": float(r),
                "threshold": float(thr), "duration_s": dur})
print(f"  F1={f1:.4f}  AUPRC={au:.4f}  P={p:.4f}  R={r:.4f}  "
...
save_artefacts(f"{base_dir}/local", scores_loc, scores_loc,
               results[-1], {"method": "local", "dataset": "seq_FUN",
                             "epochs": epochs_loc, "seed": 0})

# ===================================================================
print("\n" + "=" * 75)
print("FINAL RESULTS — seq_FUN (529 features, 500 nodes, 3919 samples)")
print("=" * 75)
print(f"{'Method':<18} {'Micro-F1':>10} {'AUPRC':>10} {'Precision':>10} "
...
for r in sorted(results, key=lambda x: -x["micro_f1"]):
    ...
    print(f"{r['method']:<18} {r['micro_f1']:>10.4f} {r['auprc']:>10.4f} "
