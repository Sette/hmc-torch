...
def load_data(name):
    ...
    g = train.g; adj = mgr.a; n_nodes = len(mgr.nodes_idx)
...
def run_go(name, X_tr, y_tr, X_te, y_te, eval_mask, g, n_nodes):
    ...
    r_matrix = torch.eye(n_nodes).unsqueeze(0).to(DEVICE)  # identity, sparse R at inference
...
for ds in ["church_FUN", "pheno_FUN"]:
    ...
    try:
        ...
        print(f"  {X_tr.shape[0]} train | {X_te.shape[0]} test | {X_tr.shape[1]} feat | {n} nodes")
        f1, au, p, r, thr, dur, ep = run_fun(ds, X_tr, y_tr, X_te, y_te, em, adj, n)
        all_results.append({"dataset": ds, "method": "global", "F1": float(f1),
                            "AUPRC": float(au), "P": float(p), "R": float(r),
                            "thr": float(thr), "time": dur, "epochs": ep, "nodes": n})
        print(f"  => F1={f1:.4f} AUPRC={au:.4f} P={p:.4f} R={r:.4f} t={dur:.1f}s ep={ep}")
...
for ds in ["gasch1_GO", "gasch2_GO", "seq_GO", "spo_GO", "pheno_GO"]:
    ...
    try:
        ...
        all_results.append({"dataset": ds, "method": "sparse_R", "F1": float(f1),
                            "AUPRC": float(au), "P": float(p), "R": float(r),
                            "thr": float(thr), "time": dur, "epochs": ep, "nodes": n})
        print(f"  => F1={f1:.4f} AUPRC={au:.4f} P={p:.4f} R={r:.4f} t={dur:.1f}s ep={ep}")
    except Exception as e:
...
for family in ["FUN", "GO"]:
    ...
    for r in sorted(ds_list, key=lambda x: -(x.get("F1") or 0)):
        if "error" in r:
            ...
            print(f"  {r['dataset']:<20} F1={r['F1']:.4f} AUPRC={r['AUPRC']:.4f} "
